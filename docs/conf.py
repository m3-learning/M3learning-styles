...
def pull_customizations():
    customizations_path = os.path.join(__location__, "../customizations")
    if os.path.isdir(customizations_path):
        try:
            subprocess.run(
                ["git", "-C", customizations_path, "pull"],
                check=True,
            )
            print("Customizations pulled successfully.")
        except subprocess.CalledProcessError as e:
            print(f"Failed to pull customizations: {e}")
    else:
        print("Customizations directory not found; skipping pull.")

# Pull customizations before any build operations
pull_customizations()
...
project = os.getenv("DOC_PROJECT_NAME", "Default Project")
...
# The extensions to use for the documentation
...
# Parse SOURCE_SUFFIX from environment, with a default configuration if not set
source_suffix = json.loads(
    os.getenv(
        "SOURCE_SUFFIX",
        '{" .rst": "restructuredtext", ".md": "markdown", ".ipynb": "notebook"}'
    )
)
...
module_dir = os.path.join(__location__,f "../src/{model_dir}")
try:
...
try:
    from m3learning_styles import __version__ as version
except ImportError:
    version = ""
...
latex_documents = [
    ("index", "user_guide.tex", f"{project} Documentation", "jagar2", "manual")
]
